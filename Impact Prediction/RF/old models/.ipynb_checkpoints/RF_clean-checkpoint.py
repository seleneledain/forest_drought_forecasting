...
now = datetime.now()
dt_string = 'RF_clean_'+now.strftime("%d/%m/%Y")
#dt_string = 'debug'
...
def get_val_metrics(rf, batch_size, n_batches_val, sample_type, exp, n_timesteps_in, n_timesteps_out, feature_set, remove_band):
    ...
    for batch_nbr in range(n_batches_val):
        ...
        mse_tot += rf.score(X_batch, y_batch)
...
for epoch in tqdm(range(n_epochs)):
    for batch_nbr in range(n_batches):
        ...
        r2, mse = get_val_metrics(grid_search, batch_size, n_batches_val, sample_type, exp, n_timesteps_in, n_timesteps_out, feature_set, remove_band)
...
r2, mse = get_test_metrics(grid_search, batch_size, n_batches_te, sample_type, exp, n_timesteps_in, n_timesteps_out, feature_set, remove_band)
...
#jbsub -q x86_24h -cores 1x1+1 -mem 64G -out sj_out/RF/RF_clean.stdout -err sj_out/RF/RF_clean.stderr python RF_clean.py