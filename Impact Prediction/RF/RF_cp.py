...
now = datetime.now()
dt_string = 'RF_cp_'+now.strftime("%d/%m/%Y")
#dt_string = 'debug'
...
for epoch in tqdm(range(n_epochs)):
    for batch_nbr in range(n_batches):
        ...
        client.log_metric(mlflow_run_id_val,"r2 batch", r2, step=epoch*n_batches + batch_nbr)
        client.log_metric(mlflow_run_id_val,"mse batch", mse, step=epoch*n_batches + batch_nbr)
    
    
    r2, mse = get_val_metrics(rf, batch_size, n_batches_val, sample_type, exp, n_timesteps_in, n_timesteps_out, feature_set, remove_band)
    print(f'Validation metrics for epoch {epoch}: R2 {r2}. MSE {mse}.') 
    client.log_metric(mlflow_run_id_val,"r2 epoch", r2, step=epoch)
    client.log_metric(mlflow_run_id_val,"mse epoch", mse, step=epoch)
...
#jbsub -q x86_24h -cores 1x8+1 -mem 64G -out sj_out/RF/RF_cp.stdout -err sj_out/RF/RF_cp.stderr python RF_cp.py