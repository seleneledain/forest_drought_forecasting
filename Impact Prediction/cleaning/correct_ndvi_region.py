...
def correct_ndvi_region(sample_type, exp_raw, exp, cp_idx, b2_idx, ndvi_idx, b2_thresh, ts_len, len_preds, split, region):
    """
    ...
    :param region: "jura", "vaud", "valais", "schaff"
    """
    ...
    process_sample(path_to_data, path_save, sample_type, split,  cp_idx, b2_idx, ndvi_idx, b2_thresh, ts_len, len_preds, all_loc_dates, full_date_range, region)
...
def process_sample(path_to_data, path_save, sample_type, split,  cp_idx, b2_idx, ndvi_idx, b2_thresh, ts_len, len_preds, all_loc_dates, full_date_range, region):
    
    samples_dir = f"{path_to_data}{split}/x/"
    len_dataset = len([f for f in os.listdir(samples_dir)])
    
    with ThreadPoolExecutor() as executor:
        future_to_sample = {executor.submit(process_sample_impl, path_to_data, path_save, sample_type, split, cp_idx, b2_idx, ndvi_idx, b2_thresh, ts_len, len_preds, all_loc_dates, full_date_range, len_dataset, region, sample): sample for sample in os.listdir(samples_dir)}
...
def process_sample_impl(path_to_data, path_save, sample_type, split, cp_idx, b2_idx, ndvi_idx, b2_thresh, ts_len, len_preds, all_loc_dates, full_date_range, len_dataset, region, sample):
    ...
    sample_clean_x, sample_clean_y = correct_noisy_s2(sample_x, sample_y, cp_idx, b2_idx, ndvi_idx, b2_thresh, ts_len, len_preds, sample_nbr, all_loc_dates, full_date_range, len_dataset, region)
...
def correct_noisy_s2(img, label, cp_idx, b2_idx, ndvi_idx, b2_thresh, ts_len, len_preds, sample_nbr, all_loc_dates, full_date_range, len_dataset, region):
    """
        ...
        for band in range(16):
            if band != cp_idx and band != b2_idx:
                ...
                if (to_rep[0] or to_rep[-1]): # If any is true
                    ...
                    band_vec = adjust_start_end_bounds(band_vec, to_rep, sample_nbr, all_loc_dates, full_date_range, len_dataset, band, region)
                    to_rep[0] = False
                    to_rep[-1] = False
...
def get_ndvi_at_day(timestamp_idx, band, region):
    """
    ...
    if band==0: # MSI
        ndvi_signal = np.load(f'corrected_bands/correct_msi_5_{region}.npy') #saved as np array
    if band==1: # NDMI
        ndvi_signal = np.load(f'corrected_bands/correct_ndmi_5_{region}.npy') #saved as np array
    if band==2: # NDVIre
        ndvi_signal = np.load(f'corrected_bands/correct_ndvire_5_{region}.npy') #saved as np array
    if band==3: # NDWI
        ndvi_signal = np.load(f'corrected_bands/correct_ndwi_5_{region}.npy') #saved as np array
    if band==4: # B11
        ndvi_signal = np.load(f'corrected_bands/correct_b11_5_{region}.npy') #saved as np array
    if band==5: # B12
        ndvi_signal = np.load(f'corrected_bands/correct_b12_5_{region}.npy') #saved as np array
    if band==7: # B3
        ndvi_signal = np.load(f'corrected_bands/correct_b3_5_{region}.npy') #saved as np array
    if band==8: # B4
        ndvi_signal = np.load(f'corrected_bands/correct_b4_5_{region}.npy') #saved as np arra
    if band==9: # B5
        ndvi_signal = np.load(f'corrected_bands/correct_b5_5_{region}.npy') #saved as np array
    if band==10: # B6
        ndvi_signal = np.load(f'corrected_bands/correct_b6_5_{region}.npy') #saved as np array
    if band==11: # B7
        ndvi_signal = np.load(f'corrected_bands/correct_b7_5_{region}.npy') #saved as np array
    if band==12: # B8
        ndvi_signal = np.load(f'corrected_bands/correct_b8_5_{region}.npy') #saved as np array
    if band==13: # B8a
        ndvi_signal = np.load(f'corrected_bands/correct_b8a_5_{region}.npy') #saved as np array
    if band==15: # NDVI
        ndvi_signal = np.load(f'corrected_bands/correct_ndvi_5_{region}.npy') #saved as np array
    
    
    return ndvi_signal[timestamp_idx] 


def adjust_start_end_bounds(vec, to_rep, sample_nbr, all_loc_dates, full_date_range, len_dataset, band, region):
    """
    ...
    if to_rep[0]:
        ...
        timestamp = datetime.strptime(start_date_sample, '%Y-%m-%d')
        timestamp_idx = full_date_range.index(timestamp)
        ndvi_correct = get_ndvi_at_day(timestamp_idx, band, region)
        vec[0] = ndvi_correct
    if to_rep[-1]:
        end_date_sample = all_loc_dates[idx_sample][3][0]
        timestamp = datetime.strptime(end_date_sample, '%Y-%m-%d')
...
correct_ndvi_region(args.sample_type, args.exp_raw, args.exp, args.cp_idx, args.b2_idx, args.ndvi_idx, args.b2_thresh, args.ts_len, args.len_preds, args.split, args.region)

#python correct_ndvi_region.py --sample_type pixel_data --exp_raw nofilter --exp clean_region --cp_idx 14 --b2_idx 6 --ndvi_idx 15 --b2_thresh 0.1 --ts_len 9 --len_preds 1 --split test_vaud --region vaud
