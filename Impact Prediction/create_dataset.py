...
def create_dataset(split, sample_type, exp, start_idx, config_id, samples_per_job, name_start_idx):
    """
    ...
    norm_stats = load_dataset_stats(sample_type, exp)

    # -------------------------------- Normalise data and set-up dataloaders-------------------------
    print("==> Creating normalised dataset...")
           

    if split=='train':
        # Dataset
        ds_norm = DroughtImpactDataset(s2_path=s2_path, era_path=era_path, dem_path=dem_path, env_path=env_path, focus_list=focus_list_train,
                              focus_time=[focus_time_train], ts_delta=ts_delta, ts_len=ts_len, ratio=ratio, len_preds=len_preds, feature_set=feature_set, agg_funct_dict=agg_funct_dict, norm_stats=norm_stats, norm_method=sim_params["norm_method"],multiple_labels=multiple_labels, correct_ndvi=correct_ndvi)
        # Sampler
        train_sampler_norm = DroughtImpactSampler(ds_norm, size=sampler_size, length=batch_size_tr*n_batch, replacement=sampler_replacement, 
                                         mask_dir=mask_dir, set_seed=sampler_set_seed, roi=sampler_roi_train, mask_threshold=mask_threshold, static_dir=static_dir)
        # Loader
        DL_norm = DataLoader(ds_norm, sampler=train_sampler_norm) 
        
        #save_called_samples(DL_norm, sample_type, 'train', exp, config_id, start_idx)
        
    if split=='val':
        norm_stats = load_dataset_stats(sample_type, exp)
        
        # Dataset
        ds_norm = DroughtImpactDataset(s2_path=s2_path, era_path=era_path, dem_path=dem_path, env_path=env_path, focus_list=focus_list_val,
                              focus_time=[focus_time_val], ts_delta=ts_delta, ts_len=ts_len, ratio=ratio, len_preds=len_preds, feature_set=feature_set, agg_funct_dict=agg_funct_dict, norm_stats=norm_stats, norm_method=sim_params["norm_method"], multiple_labels = multiple_labels, correct_ndvi=correct_ndvi)
        # Sampler
        val_sampler_norm = DroughtImpactSampler(ds_norm, size=sampler_size, length=sampler_length_val, replacement=sampler_replacement, 
                                         mask_dir=mask_dir, set_seed=sampler_set_seed, roi=sampler_roi_val, mask_threshold=mask_threshold, static_dir=static_dir)
        # Loader
        DL_norm = DataLoader(ds_norm, sampler=val_sampler_norm) 
        
        #save_called_samples(DL_norm, sample_type, 'val', exp, config_id, start_idx)
    
    if 'test' in split: # could also provide a split called test2
        norm_stats = load_dataset_stats(sample_type, exp)
        
        # Dataset
        ds_norm = DroughtImpactDataset(s2_path=s2_path, era_path=era_path, dem_path=dem_path, env_path=env_path, focus_list=focus_list_test,
                              focus_time=[focus_time_test], ts_delta=ts_delta, ts_len=ts_len, ratio=ratio, len_preds=len_preds, feature_set=feature_set, agg_funct_dict=agg_funct_dict, norm_stats=norm_stats, norm_method=sim_params["norm_method"], multiple_labels = multiple_labels, correct_ndvi=correct_ndvi)
        # Sampler
        test_sampler_norm = DroughtImpactSampler(ds_norm, size=sampler_size, length=sampler_length_te, replacement=sampler_replacement, 
                                         mask_dir=mask_dir, set_seed=sampler_set_seed, roi=sampler_roi_test, mask_threshold=mask_threshold, static_dir=static_dir)
        # Dataloader
        DL_norm = DataLoader(ds_norm, sampler=test_sampler_norm)
        
        #save_called_samples(DL_norm, sample_type, 'test', exp, config_id, start_idx)
    ...
    # Create data samples
    create_datacube(sample_type=sample_type, exp=exp, dataloader=DL_norm, split=split, remove_bands=remove_bands, feature_set=ds_norm.feature_set,
                     c_features=len(ds_norm.feature_set)-len(remove_bands), t_features=model_params["output_dim"], 
                     size=sampler_size, ts_len=ts_len, len_preds=len_preds, start_idx=start_idx, multiple_labels=ds_norm.multiple_labels, samples_per_job=samples_per_job, name_start_idx=name_start_idx)
...
create_dataset(args.split, args.sample_type, args.exp, args.start_idx, args.config_id, args.samples_per_job, args.name_start_idx)
