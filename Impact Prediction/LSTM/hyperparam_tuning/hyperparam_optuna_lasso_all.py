...
config_id = 'train_all_config_clean_update'
...
now = datetime.now()
dt_string = 'resume_lasso_optuna_all_'+now.strftime("%d/%m/%Y")
...
def objective(trial):
    ...
    hidden_dim = 32
    l1_lambda = trial.suggest_loguniform("l1_lambda", 0.0001, 100)
    l1_loss =  nn.L1Loss()
...
#optuna.delete_study(study_name="tuning-20-02-2023-lasso", storage="sqlite:///example-study.db")

study = optuna.create_study(
        storage="sqlite:///example-study.db",  # Specify the storage URL here.
        study_name="tuning-21-02-2023-lasso",
...
#jbsub -q x86_24h -cores 1x8+1 -mem 128G -out sj_out/optuna_lasso_all.stdout -err sj_out/optuna_lasso_all.stderr python hyperparam_optuna_lasso_all.py 
