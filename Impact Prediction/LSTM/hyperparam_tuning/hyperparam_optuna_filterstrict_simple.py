...
config_id = 'train_config_clean_filter_strict'
...
now = datetime.now()
dt_string = 'optuna_filterstrict_simple_'+now.strftime("%d/%m/%Y")
    ...
    for ix_epoch in range(10): 
        ...
        for batch_nbr in range(n_batch): 
            ...
            for sample in range(batch_size_tr):
                y_true = th.cat([img[sample,:,0,:,:], label[sample,:,:,:]]).squeeze(1).squeeze(1).to(device)
                y_pred_all = th.cat([img[sample,:,0,:,:].squeeze(1).squeeze(1).to(device), preds[sample][0]]).to(device)
...
#optuna.delete_study(study_name="tuning-15-02-2023-filtersimple", storage="sqlite:///example-study.db")

study = optuna.create_study(
        storage="sqlite:///example-study.db",  # Specify the storage URL here.
        study_name="tuning-20-02-2023-filtersimple",
...
#jbsub -q x86_24h -cores 1x8+1 -mem 128G -out sj_out/optuna_filterstrict.stdout -err sj_out/optuna_filterstrict.stderr python hyperparam_optuna_filterstrict_simple.py 
