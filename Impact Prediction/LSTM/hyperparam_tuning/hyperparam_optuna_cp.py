...
config_id = 'train_config_nofilter_cp'
...
now = datetime.now()
dt_string = 'resume_optuna_cp_'+now.strftime("%d/%m/%Y")
...
def objective(trial):
    ...
    for ix_epoch in range(10): 
        ...
        for batch_nbr in range(n_batch): 
            ...
            to_keep = 1 # NDVI only
            label = label[:,:,to_keep,:,:] # keep CP in label, but still just predict NDVI (n_feats_out stays 1)
            ...
            condition = cp_mask < 0.01
            preds_ndvi_masked = th.masked_select(preds, condition.to(device))
            y_ndvi_masked = th.masked_select(y[:,:,0].unsqueeze(1), condition.to(device))
            if preds_ndvi_masked.nelement() == 0:
                ...
                loss = criterion(preds_ndvi_masked.to(device), y_ndvi_masked.to(device))
                r2_batch = 0 
                # Calculate R2 per sample (if not masked)
                for sample in range(batch_size_tr):
                    if condition[sample]:
                        y_true = th.cat([img[sample,:,1,:,:], label[sample,:,:,:]]).squeeze(1).squeeze(1).to(device)
                        y_pred_all = th.cat([img[sample,:,1,:,:].squeeze(1).squeeze(1).to(device), preds[sample][0]]).to(device)
    ...
    with th.no_grad():
        ...
        for batch_nbr in range(n_batch_val):
            ...
            if preds_ndvi_masked.nelement() == 0:
                ...
                total_loss += criterion(preds_ndvi_masked.to(device), y_ndvi_masked.to(device))
                r2_batch = 0 
                # Calculate R2 per sample (if not masked)
                for sample in range(batch_size_val):
                    if condition[sample]:
                        ...
                        y_pred_all = th.cat([img[sample,:,1,:,:].squeeze(1).squeeze(1).to(device), preds[sample][0]]).to(device)
                        r2_batch += r2_score(y_true, y_pred_all)
                r2 = r2_batch/batch_size_val
                total_r2 += r2
                client.log_metric(mlflow_run_id,"val r2", r2, step=ix_epoch*n_batch_val+batch_nbr)
...
#optuna.delete_study(study_name="tuning-15-02-2023-cp", storage="sqlite:///example-study.db")

study = optuna.create_study(
        storage="sqlite:///example-study.db",  # Specify the storage URL here.
        study_name="tuning-15-02-2023-cp",
...
#jbsub -q x86_24h -cores 1x8+1 -mem 128G -out sj_out/optuna_cp_simple.stdout -err sj_out/optuna_cp_simple.stderr python hyperparam_optuna_cp.py 
