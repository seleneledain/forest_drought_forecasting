...
def train_simple_exp(method, num_layers, hidden_dim, lr, output_dim, config_id):
    ...
    if checkpoint_file is None:
        ...
        #print('Model summary', summary(model, input_size=(40,9,1)))

        # Create new MLflow experiment
        now = datetime.now()
        dt_string = now.strftime("%d/%m/%Y")+f'_{lr}_{num_layers}_{hidden_dim}'
        #dt_string = 'debug'
    ...
    for ix_epoch in tqdm(range(sim_params["num_epochs"])): 
        ...
        # Train
        epoch_loss, epoch_r2 = train_model(method=method, model=model, epoch=ix_epoch, loss_function=criterion, optimizer=optimizer, 
                                 ...
                                 experiment=experiment, checkpoint_folder=checkpoint_folder, dt_string=dt_string, start_batch=start_batch, client=client, run_id=mlflow_run_id, epoch_loss=epoch_loss, epoch_r2=epoch_r2,
                                 sample_type=sample_type, exp=exp, cp_idx=cp_idx)
        ...
        val_loss, val_r2 = test_model(method=method, model=model, epoch=ix_epoch, loss_function=criterion, 
                                 ...
                                 sample_type=sample_type, exp=exp_val, cp_idx=cp_idx, dt_string=dt_string)
    ...
    test_loss, test_r2 = test_model(method=method, model=model, epoch=ix_epoch, loss_function=criterion, 
                                 ...
                                 sample_type=sample_type, exp=exp_te, cp_idx=cp_idx, dt_string=dt_string)
...
# python train_simple_exp.py --method dir --num_layers 1 --hidden_dim 10 --lr 0.001 --output_dim 1 --config_id train_config_nofilter
