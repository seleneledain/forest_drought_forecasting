...
from model_all_dropfeat import *
...
def train_all(method, num_layers, hidden_dim, lr, output_dim, config_id, ndvi_idx, feature_imp=None):
    ...
    if checkpoint_file is not None:
        ...
        if method == 'dir': #direct
            model = LSTM_oneshot(input_dim=len(feature_set)-len(remove_bands)-1, hidden_dim=hidden_dim, num_layers=num_layers, output_dim=output_dim, device=device)
    ...
    if checkpoint_file is None:
        ...
        now = datetime.now()
        dt_string = f'f{feature_imp}_{lr}_{num_layers}_{hidden_dim}'
        #dt_string = 'debug'
    ...
    for ix_epoch in tqdm(range(sim_params["num_epochs"])): 
        ...
        total_loss, epoch_r2 = train_model(method=method, model=model, epoch=ix_epoch, loss_function=criterion, optimizer=optimizer, 
                                 ...
                                 n_timesteps_in=ts_len, n_timesteps_out=1, n_feats_in=len(feature_set)-len(remove_bands)-1, n_feats_out=output_dim, 
                                 ...
                                 sample_type=sample_type, exp=exp, cp_idx=cp_idx, device=device, ndvi_idx=ndvi_idx, feature_imp=feature_imp)
        ...
        val_loss, val_r2 = test_model(method=method, model=model, epoch=ix_epoch, loss_function=criterion, 
                                 ...
                                 sample_type=sample_type, exp=exp_val, cp_idx=cp_idx, dt_string=dt_string, device=device, ndvi_idx=ndvi_idx, feature_imp=feature_imp)
    ...
    test_loss, test_r2 = test_model(method=method, model=model, epoch=ix_epoch, loss_function=criterion, 
                                 ...
                                 sample_type=sample_type, exp=exp_te, cp_idx=cp_idx, dt_string=dt_string, device=device, ndvi_idx=ndvi_idx, feature_imp=feature_imp)
...
#jbsub -q x86_24h -cores 1x8+1 -mem 128G -out sj_out/feat/f13_filter_0.0001041_2_32.stdout -err sj_out/feat/f13_filter_0.0001041_2_32.stderr python train_all_clean_filter_dropfeat.py --method dir --num_layers 2 --hidden_dim 32 --lr 0.0001041 --output_dim 1 --config_id train_all_config_clean_filter --ndvi_idx 13 --feature_imp 13

#jbsub -q x86_24h -cores 1x8+1 -mem 128G -out sj_out/feat/f40_filter_0.0001041_2_32.stdout -err sj_out/feat/f40_filter_0.0001041_2_32.stderr python train_all_clean_filter_dropfeat.py --method dir --num_layers 2 --hidden_dim 32 --lr 0.0001041 --output_dim 1 --config_id train_all_config_clean_filter --ndvi_idx 14 --feature_imp 41