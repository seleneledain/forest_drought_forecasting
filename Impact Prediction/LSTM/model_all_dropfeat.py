...
def train_model(method, model, epoch, loss_function, optimizer, batch_size, n_batch, n_timesteps_in,  n_timesteps_out, n_feats_in, n_feats_out, remove_band, feature_set, experiment, checkpoint_folder, dt_string, start_batch, client, run_id, total_loss, total_r2, sample_type, exp, cp_idx, device, ndvi_idx, feature_imp=None):
    """
    ...
    :param ndvi_idx: what index the NDVI layer can be found
    ...
    to_drop = remove_band + [feature_set[feature_imp]]
    ...
    for batch_nbr in range(n_batch):
        ...
        img = remove_features_batch_level(img, to_drop, feature_set)
        if 'NDVI' in to_drop: #we still need NDVI in the label
            label = remove_features_batch_level(label, remove_band, feature_set)
        else:
            label = remove_features_batch_level(label, to_drop, feature_set)
        ...
        if cp_idx:
            ...
            for sample in range(batch_size):
                y_true = th.cat([img[sample,:,ndvi_idx,:,:], label[sample,:,:,:]]).squeeze(1).squeeze(1).to(device)
                y_pred_all = th.cat([img[sample,:,ndvi_idx,:,:].squeeze(1).squeeze(1).to(device), preds[sample][0]]).to(device)
    ...
    #client.log_metric(run_id,"avg epoch r2", epoch_r2/n_batch, step=epoch)
     print(f"Epoch nbr {epoch}. avg epoch loss: {epoch_loss/n_batch}")
...
def test_model(method, model, epoch, loss_function, batch_size, n_batch, n_timesteps_in,  n_timesteps_out, n_feats_in, n_feats_out, remove_band, feature_set, experiment, split, start_batch, client, run_id, sample_type, exp, cp_idx, dt_string, device, ndvi_idx, feature_imp, checkpoint_folder=None):
    """
    Train model in one epoch
    ...
    :param exp: experiment name (for where the data is stored)
    :param cp_idx: tuple containing index of where the cp layer and ndvi is in the label/predicted tensor, so that it can be used to mask results
    :param dt_string: name of run/experiment for MLflow tracking
    :param device: cpu or gpu
    :param ndvi_idx: index of NDVI layer in tensors
    """
    # Use the GPU for computations
    model = model.to(device)
    to_drop = remove_band + [feature_set[feature_imp]]
    ...
    print(f" Average total loss: {total_loss/n_batch}")
